...
function restore_pstore() {
  pstore_file=/data/oppo/log/pstore/pstore_last_kmsg
  filename=/data/oppo/log/DCS/de/AEE_DB/last_kmsg
  de_filename=/data/oppo/log/DCS/de/AEE_DB/AEE_SYSTEM_LAST_KMSG
  aee_type_ori="`getprop vendor.debug.mtk.aeev.db`"
  aee_type=`echo ${aee_type_ori##*.} `
  uuid="`uuidgen | sed 's/-//g'`"
  ota_version="`getprop ro.build.version.ota`"
  timestamp="`date "+%Y-%m-%d-%H-%M-%S"`"
  tar_name=${de_filename}@${uuid}@${ota_version}@${timestamp}.dat

  rm -rf /data/oppo/log/DCS/de/AEE_DB/*
  if [ -f ${pstore_file} ]; then
    cp ${pstore_file} ${filename}
    tar -cf ${tar_name} ${filename}
    gzip ${tar_name}
    rm -f ${filename}
    try_copy_minidump_to_opporeserve ${tar_name}.gz
  fi
}
